...
import { QR_CONFIG } from './player.types';
...
describe('Player', () => {
  ...
  it('renders the initial QR code', () => {
    ...
    expect(qrCode.dataset.size).toBe(String(QR_CONFIG.CODE_SIZE));
  });
  ...
  it('cycles the QR code at the configured refresh rate', () => {
    ...
    renderWithProvider(<Player {...defaultProps} cbor={largeCbor} />);

    const initialValue = screen.getByTestId('qr-code-svg').dataset.value;

    act(() => {
      jest.advanceTimersByTime(QR_CONFIG.REFRESH_RATE);
    });
  ...
  it('cleans up the interval on unmount to prevent memory leaks', () => {
    ...
    const { unmount } = renderWithProvider(<Player {...defaultProps} />);
  ...
  it('produces new QR output when cbor changes', () => {
    const { rerender } = renderWithProvider(<Player {...defaultProps} />);
