...
export async function proxyTronBlockchainCalls(
  ...
  const proxyGetPath = async (path: string) => {
    endpoints.push(
      await mockServer
        ...
        .always()
        .thenCallback(async () => proxyGet(localNodeUrl, path)),
    );
  };
  ...
  // TODO: Clarify the intent of this as the current code
  // uses mocks for tokens in 'seededTrc20ContractAddresses'
  // instead of proxying them to the node.
  // Using an empty set in the meantime to disable this behavior.
  // const seededTrc20ContractAddresses =
  //   buildSeededTrc20ContractAddressSet(localNode);
  const seededTrc20ContractAddresses = new Set();
